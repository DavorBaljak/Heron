...
**Loxone connection setup** (`ensureLoxoneConnection` in `packages/agent/src/index.ts`): there's no reliable Loxone auto-discovery on a local network, so if `LOXONE_HOST`/`LOXONE_USER`/`LOXONE_PASSWORD` aren't already set (via `.env` or the shell — checked first, so Docker's env-var config keeps working unchanged), the agent asks for them interactively on first run and saves them to `packages/agent/data/loxone-config.json` (`loxoneConfig.ts`, written with `0600` permissions — still plaintext on disk, just not world-readable) so it isn't asked again. To reconfigure, delete that file (or set `HERON_LOXONE_CONFIG_PATH` to point elsewhere) and restart.
...
- `npm run dev --workspace=@heron/agent` — start the CLI agent; requires `ANTHROPIC_API_KEY`. `LOXONE_HOST`/`LOXONE_USER`/`LOXONE_PASSWORD` are optional — if unset, the agent prompts for them interactively and remembers the answer (see the Loxone connection setup note above). Optional `ANTHROPIC_MODEL` (default `claude-sonnet-5`), `MCP_SERVER_ENTRY` to point at a different mcp-server entry file, `HERON_LOXONE_CONFIG_PATH`/`HERON_DISCOVERY_CACHE_PATH`/`HERON_ACTION_LOG_PATH` to relocate the agent's on-disk state.
